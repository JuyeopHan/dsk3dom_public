...
public:
    myMap(std::string map_topic,
        ...
        : grid_map(grid_params), lidar_frame(lidar_frame),
          ...
          initial(true), br(), listener(), count(0), min_z(min_z), max_z(max_z),
    ...
    void publish_map() {
        ...
        for (int i = 0; i < grid_map.grid_cell_count; ++i) {
            if (grid_map.grid_cell_array_host[i].occ_prob() > min_occ_threshold &&
                grid_map.grid_cell_array_host[i].occ_mass + grid_map.grid_cell_array_host[i].free_mass > min_mass_threshold) {
                ...
                if (map_vis_mode == 0) {    // classification result
                    speed = sqrtf(grid_map.grid_cell_array_host[i].mean_vel_sq()) * grid_map.getResolution();
                    m_pub.insert_point3d_class(x, y, z, speed, 1.0f);
                } else { // height map
    ...
    void publish_mapdata() {
        ...
        for (int i = 0; i < grid_map.grid_cell_count; ++i) {
            ...
            pt.occ_val = grid_map.grid_cell_array_host[i].occ_prob();
            pt.dyn_val = grid_map.grid_cell_array_host[i].mean_vel_sq() * res * res;    // speed^2
            pt.eval_aug = -(1.0f - grid_map.grid_cell_array_host[i].occ_mass - grid_map.grid_cell_array_host[i].free_mass);
            pt.dyn_aug = grid_map.grid_cell_array_host[i].occ_mass;
...
{
    ros::init(argc, argv, "dsphdmib_demo_node");
    ros::NodeHandle nh("~");
    ...
    float valid_thresh = 0.1f;
    float min_z = 0.0f;
    float max_z = 0.0f;
    int map_pub_freq = 5;
    float imu_pose_offset = 0;
    float size = 10.0f;
    float size_z = 5.0f;
    float mass_scale = 3.0f;
    float meas_occ_stddev = 0.6f;
    bool do_eval = false;
    int map_vis_mode = 0;
    ...
    float particle_min_vel = 0.0f;

    // initial evidence
    float prior_free = 0.0f;
    float prior_occ = 0.0f; 
    float prior_static = 0.0f;  // dummy for compatability with ds-phd/mib
    float prior_dynamic = 0.0f; // dummy for compatability with ds-phd/mib
    float prior_all = 0.0f; // dummy for compatability
    ...
    nh.param<float>("occupied_thresh", valid_thresh, valid_thresh);
    ...
    nh.param<float>("meas_occ_stddev", meas_occ_stddev, meas_occ_stddev);
    ...
    ROS_INFO_STREAM("Parameters:" << std::endl <<
            ...
            "meas_occ_stddev: " << meas_occ_stddev << std::endl <<
    ...
    grid_params.max_range = 50.0f;
    grid_params.meas_occ_stddev = meas_occ_stddev;

    // Just to init cuda
    cudaDeviceSynchronize();

    dom::myMap mymap(map_topic, lidar_frame, occupied_thresh, valid_thresh, min_z, max_z, map_pub_freq, imu_pose_offset, do_eval, map_vis_mode, nh, grid_params);
