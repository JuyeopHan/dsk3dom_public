...
namespace dom
{
// GridCell: one grid cell containing parameters
struct GridCell
{
    int start_idx; // start index in particle array of the cell
    int end_idx; // end index in particle array of the cell

    float mu_A; // normalization component for the case of an associated measurement
    float mu_UA; // normalization component for the case of an unassociated measurement

    float w_A; // weight of an associated new particle
    float w_UA; // weight of an unassociated new particle
    ...
    float likelihood; // measurement likelihood
    float p_A; // association probability of measurement
    ...
    For ds-k3dom,
        free/static/dynamic/occ(={D,S})/total(있어야 할 지?): evidence in Dempster-Shafer theory
        pers_mass: total (dynamic) mass of persistent particles (dynamic_mass = pers_mass + mass for new born particles)
        pred_mass: total (dynamic) mass of newly incident particles before mass update
    */
    ...
    // for ds-k3dom
    inline float dyn_belief() const {return dynamic_mass + 0.5f * occ_mass + std::max(0.0f, 0.3333f * (1.0f - total_belief()));}
    inline float occ_belief() const {return static_mass + dynamic_mass + occ_mass + std::max(0.0f, 0.6666f * (1.0f - total_belief()));}
    inline float total_belief() const {return static_mass + dynamic_mass + free_mass + occ_mass;}
...
// MeasurementCell: contains measurement masses
