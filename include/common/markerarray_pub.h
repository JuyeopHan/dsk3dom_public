...
namespace dom {
    ...
    public:
        ...
        // for ds-k3dom
        void insert_point3d_class(float x, float y, float z, float f_mass, float s_mass, float d_mass, float occ_mass){
            float total = s_mass + f_mass + d_mass + occ_mass;
            float all_mass = std::max(0.0f, 1.0f - total);
            float pignistic_static = s_mass + 0.5f * occ_mass + 0.3333f * all_mass;
            float pignistic_dynamic = d_mass + 0.5f * occ_mass + 0.3333f * all_mass;
            //float pignistic_free = f_mass + 0.3333f * all_mass;
            ...
            std_msgs::ColorRGBA color;
            color.r = 0.0f; // free
            color.g = pignistic_static; // static
            color.b = pignistic_dynamic; // dynamic
            color.a = 1;

            map_msg->markers[depth].colors.push_back(color);
        }

        void insert_point3d_var(float x, float y, float z, float f_mass, float s_mass, float d_mass, float occ_mass) {
